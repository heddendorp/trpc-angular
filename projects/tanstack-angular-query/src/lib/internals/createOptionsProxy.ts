...
export function createTRPCOptionsProxy<TRouter extends AnyTRPCRouter>(
  ...
  const client = (() => {
    ...
    const mockClient = {
      query: (path: string, input: unknown, options?: TRPCRequestOptions) => {
        const ctx = typeof opts.ctx === 'function' ? (opts.ctx as () => any)() : opts.ctx;
    ...
    return mockClient as unknown as TRPCUntypedClient<TRouter>;
  })();
  ...
  return createTRPCRecursiveProxy(({ path, args }) => {
    ...
    return createTRPCRecursiveProxy(({ path: nextPath, args: nextArgs }) => {
      // This is a hack to get around the type issue
      const proxy = createTRPCOptionsProxy(opts as any);
      return (proxy as any)({
        path: [...path, ...nextPath],
        args: nextArgs,
      });
